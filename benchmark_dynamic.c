...
void free_records(struct splay_link *root)
{
	if (!root)
		return;
	struct record *rec = SPLAY_CONTAINER(root, struct record, link);
...
int main(int argc, char **argv)
{
	...
	char **test_keys = malloc(num_tests * sizeof(*test_keys));
	size_t num_test_keys = 0;

	struct splay_link *root = NULL;

	printf("Allocating, generating, and inserting %lu random string/integer pairs\n", num_tests);
	...
	for (size_t i = 0; i < num_tests; i++) {
		...
		rand_string(rec->key);
		rec->val = rand();
		test_keys[num_test_keys++] = rec->key;
	...
	int *rand_indices = malloc(num_test_keys * sizeof(*rand_indices));
	...
	for (size_t i = 0; i < num_tests; i++) {
		char *test_key = test_keys[rand_indices[i]];
		struct record *rec = SPLAY_CONTAINER(test_key, struct record, key);
	...
	for (size_t i = 0; i < num_tests; i++) {
		char *test_key = test_keys[rand_indices[i]];
		struct record *rec = SPLAY_CONTAINER(test_key, struct record, key);
		struct splay_link *del_link = splay_delete(&root, record_nav, &rec->link);
		if (del_link == NULL) {
			printf("Deletion failed\n");
			printf("Word was: %s\n", rec->key);
			free(rec);
		} else {
			struct record *del_rec = SPLAY_CONTAINER(del_link, struct record, link);
			free(del_rec);
		}
	}
